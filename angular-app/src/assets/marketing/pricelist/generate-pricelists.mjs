/**
 * Прайс-лист DOCX: логотип + таблицы (COLOR Mix, однотонная, серая, бордюры).
...
/** Серый цемент: как в УДМС — серая + однотонные на сером основании */
...
async function buildDocument(logoPng) {
  ...
  /** Однотонная (белая, синяя и др. на белом/ином цементе) — мок. 40 руб/м² */
  const monoRows = [
    ...
    sections: [
      {
        ...
        children: [
          ...
          notePara(
            'Как в прайсе УДМС: отдельные цены за м² для серой плитки и для однотонных цветов на сером цементе. Мок.: серая — 20 руб/м² (−30 руб/м² к базе Color Mix 50); чёрная, жёлтая, коричневая, красная, зелёная — по 40 руб/м².'
          ),
          makeTable(grayCementMatrixRows),
          sectionTitle('3. Однотонная плитка (белая, синяя и др.)'),
          notePara(
