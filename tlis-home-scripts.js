        ...
        function updateActiveMenuItem() {
            ...
            if (window.scrollY > 50) {
                ...
                workingHoursElements.forEach(element => {
                    element.style.fontSize = '14px'; // Уменьшаем шрифт
                });
            } else {
                header.classList.remove('scrolled');
                if (emailElement) {
                    emailElement.style.display = ''; // Сбрасываем display, чтобы вернуть его исходное значение из CSS
                }
                workingHoursElements.forEach(element => {
                    element.style.fontSize = '16px'; // Возвращаем исходный шрифт
                });
            }
